...
Partial Class FurnitureMapSouth
    ...
    Protected Function PopulateDataTables() As String  'store distinct prods with rgas and bos so you don't have to scan everything
        ...
        mysqldataadapter.SelectCommand = New MySqlCommand("Select distinct ProdNo FROM RGAs where BenRcvd is Null and DeliveredToLine is null and ProdNo > 2100000", conn)
        mysqldataadapter.Fill(rgaProdNoList)

        mysqldataadapter2.SelectCommand = New MySqlCommand("Select distinct ProdNo FROM Backorders where Cleared = 0 and DeliveredToLine is Null and ProdNo > 2100000", conn)
        mysqldataadapter2.Fill(boProdNoList)

        mysqldataadapter3.SelectCommand = New MySqlCommand("Select distinct ProdNo FROM RGAs where BenRcvd = 1 and DeliveredtoLine is Null and ProdNo > 2100000", conn)
        mysqldataadapter3.Fill(rgaProdNoListReceivedNotDeliveredToLine)


        mysqldataadapter4.SelectCommand = New MySqlCommand("Select distinct ProdNo FROM Backorders where Cleared = 1 and DeliveredToLine is null and ProdNo > 2100000 ", conn)
    ...
    Protected Sub dlRails_ItemDataBound(sender As Object, e As DataListItemEventArgs) Handles dlRails.ItemDataBound, dlHelms.ItemDataBound, dlFurniture.ItemDataBound, dlInspection.ItemDataBound, dlFinal.ItemDataBound
