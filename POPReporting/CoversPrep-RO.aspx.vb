...
Partial Class CoversPrep_RO
    ...
    Protected Sub gvAllFieldsC_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles gvAllFieldsC.RowDataBound

        If e.Row.RowType = DataControlRowType.DataRow Then

            Dim s As SqlDataSource = CType(e.Row.FindControl("sdsPopReportingByProdNo"), SqlDataSource)
            Dim t As SqlDataSource = CType(e.Row.FindControl("sdsFabOptionsCovers"), SqlDataSource)
    ...
    Protected Sub gvAllFieldsC_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvAllFieldsC.RowCommand

        If e.CommandName = "goto" Then 'handles when the notes button is clicked
            Dim ID As String = e.CommandArgument 'get the row that is clicked
            Dim Selected As Integer = gvAllFieldsC.Rows(ID).Cells(0).Text 'find the prod # in that row to use in the update command
            Dim workstation As String = "Covers" 'to pass in the workstation that makes the note.
            ...
            'Dim s As String = "window.open('" & url + "', 'popup_window', 'width=600,height=350,left=100,top=100,resizable=yes');"
        ...
        If e.CommandName = "UpdateCStatus" Then
            'This code is for the update button
            Dim Key As String = e.CommandArgument 'get the row that is clicked
            Dim SelectedProdNo As Integer = gvAllFieldsC.Rows(Key).Cells(0).Text 'find the prod # in that row to use in the update command


            Dim rbList As RadioButtonList = DirectCast(gvAllFieldsC.Rows(Key).Cells(4).FindControl("rblCoversStatus"), RadioButtonList)
            ...
            cmd.CommandText = "UPDATE BML_POPREPORTING_GREENLIGHTS SET CoversStatus = '" & rbList.SelectedValue & "' , CoversLastUpdated = '" & TodaysDate & "' where ProdNo =  '" & SelectedProdNo & "'"
            cmd2.CommandText = "Select Count (*) FROM BML_POPREPORTING_GREENLIGHTS where CoversStatus = 2 and Convert(date,CoversLastUpdated) ='" & ThisDay & "'"
            ...
            If greenlightcount = 1 Then '5 major greenlights are green
                ...
                ElseIf tubeneeded = 1 And tubecomplete = 0 Then 'if on the order and not complete, set frames to start.
                    cmd5.ExecuteNonQuery()
                ElseIf tubeneeded = 1 And tubecomplete = 1 Then ' if on the order and complete, reverse frames.
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If ddlShowSelect.SelectedValue = 1 Then
            gvAllFieldsC.Columns(6).Visible = False
        Else : gvAllFieldsC.Columns(6).Visible = True
        End If
        'MesgBox(DaystoShow)
        conn.ConnectionString = connstr
    ...
    Protected Sub ddlShowSelect_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ddlShowSelect.SelectedIndexChanged
        gvAllFieldsC.DataBind() 'used to allow the user to hide/show jobs that they have completed.
        If ddlShowSelect.SelectedValue = 1 Then
            gvAllFieldsC.Columns(6).Visible = False
        Else : gvAllFieldsC.Columns(6).Visible = True
        End If
