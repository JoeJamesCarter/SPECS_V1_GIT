...
Partial Class CenterTubes
    ...
    Protected Sub gvAllFieldsCT_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles gvAllFieldsCT.RowDataBound
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            Dim u As SqlDataSource = CType(e.Row.FindControl("sdsPopReportingFabOptionsDirectFill"), SqlDataSource)
    ...
    Protected Sub gvAllFieldsCT_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvAllFieldsCT.RowCommand

        If e.CommandName = "goto" Then 'handles when the offline/online button is clicked
            Dim ID As String = e.CommandArgument 'get the row that is clicked
            Dim Selected As Integer = gvAllFieldsCT.Rows(ID).Cells(0).Text 'find the prod # in that row to use in the update command
            Dim workstation As String = "Centertubes" 'to pass in the workstation that makes the note.
        ...
        If e.CommandName = "UpdateCTStatus" Then
            'This code is for the update button
            Dim Key As String = e.CommandArgument 'get the row that is clicked
            Dim SelectedProdNo As Integer = gvAllFieldsCT.Rows(Key).Cells(0).Text 'find the prod # in that row to use in the update command


            Dim rbList As RadioButtonList = DirectCast(gvAllFieldsCT.Rows(Key).Cells(4).FindControl("rblCentertubeStatus"), RadioButtonList)
            ...
            cmd.CommandText = "UPDATE BML_POPREPORTING_GREENLIGHTS SET CentertubeStatus = '" & rbList.SelectedValue & "' , Centertube = '" & TodaysDate & "' where ProdNo =  '" & SelectedProdNo & "'"
            'to get the count of the prod #'s completed that day as it changes.
            cmd2.CommandText = "Select Count (*) FROM BML_POPREPORTING_GREENLIGHTS where CentertubeStatus = 2 and Convert(date,Centertube) ='" & ThisDay & "'"
            'check all greenlights to see if you can start frames.
            ...
            'set frames to start and set dateallgreen.
            ...
            If rbList.SelectedValue.ToString = "2" Then
                Try
                    If gvAllFieldsCT.Rows(Key).DataItem("IsPilot").ToString = "1" Then
                        Dim modelLbl As Label = CType(gvAllFieldsCT.Rows(Key).FindControl("lblItemNo"), Label)
                        Dim model As String = modelLbl.Text
                        SpecsGlobalFunctions.EmailPilotTeam(SelectedProdNo, model, "Center Tubes are Green ")
                    End If
            ...
            Try
                If gvAllFieldsCT.Rows(Key).DataItem("PerfPkg").ToString = "SPS" Or gvAllFieldsCT.Rows(Key).DataItem("PerfPkg").ToString = "ESP" Then
                    ESP_SPS_Prompt(SelectedProdNo)
                End If
            Catch ex As Exception
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If ddlShowSelect.SelectedValue = 1 Then
            gvAllFieldsCT.Columns(8).Visible = False
        Else : gvAllFieldsCT.Columns(8).Visible = True
        End If
        'MesgBox(DaystoShow)
    ...
    Protected Sub ddlShowSelect_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ddlShowSelect.SelectedIndexChanged
        gvAllFieldsCT.DataBind() 'used to allow the user to hide/show jobs that they have completed.
        If ddlShowSelect.SelectedValue = 1 Then
            gvAllFieldsCT.Columns(8).Visible = False
        Else : gvAllFieldsCT.Columns(8).Visible = True
        End If
    ...
    Function ESP_SPS_Prompt(ProdNo As String) As String
        Dim text7 As String = "Closing This Box Verifies That Critical Welds for Prod #: " & ProdNo & " have been inspected."
        ScriptManager.RegisterStartupScript(Me, GetType(Page), "Critical Welds", "<script>showpopError('" & text7 & "')</script>", False)
        Return False
    End Function
