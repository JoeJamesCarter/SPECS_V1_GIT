...
Partial Class Harnesses
    ...
    Protected Sub gvAllFieldsH_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles gvAllFieldsH.RowDataBound

        If e.Row.RowType = DataControlRowType.DataRow Then

            Dim s As SqlDataSource = CType(e.Row.FindControl("sdsPopReportingByProdNo"), SqlDataSource)
            Dim t As SqlDataSource = CType(e.Row.FindControl("sdsPopReportingHarnessOptions"), SqlDataSource)
    ...
    Protected Sub gvAllFieldsH_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvAllFieldsH.RowCommand

        If e.CommandName = "goto" Then 'handles when the notes button is clicked
            Dim ID As String = e.CommandArgument 'get the row that is clicked
            Dim Selected As Integer = gvAllFieldsH.Rows(ID).Cells(0).Text 'find the prod # in that row to use in the update command
            Dim workstation As String = "Harnesses" 'to pass in the workstation that makes the note.
        ...
        If e.CommandName = "UpdateHStatus" Then


            'This code is for the update button
            Dim Key As String = e.CommandArgument 'get the row that is clicked
            Dim SelectedProdNo As Integer = gvAllFieldsH.Rows(Key).Cells(0).Text 'find the prod # in that row to use in the update command


            Dim rbList As RadioButtonList = DirectCast(gvAllFieldsH.Rows(Key).Cells(4).FindControl("rblHarnessStatus"), RadioButtonList)
            ...
            cmd.CommandText = "UPDATE BML_POPREPORTING_GREENLIGHTS SET HarnessStatus = '" & rbList.SelectedValue & "' , HarnessLastUpdated = '" & TodaysDate & "' where ProdNo =  '" & SelectedProdNo & "'"
            cmd2.CommandText = "Select Count (*) FROM BML_POPREPORTING_GREENLIGHTS where HarnessStatus = 2 and Convert(date,HarnessLastUpdated) ='" & ThisDay & "'"
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If ddlShowSelect.SelectedValue = 1 Then
            gvAllFieldsH.Columns(7).Visible = False
        Else : gvAllFieldsH.Columns(7).Visible = True
        End If
    ...
    Protected Sub ddlShowSelect_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ddlShowSelect.SelectedIndexChanged
        gvAllFieldsH.DataBind() 'used to allow the user to hide/show jobs that they have completed.
        If ddlShowSelect.SelectedValue = 1 Then
            gvAllFieldsH.Columns(7).Visible = False
        Else : gvAllFieldsH.Columns(7).Visible = True
        End If
