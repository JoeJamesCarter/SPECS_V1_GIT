...
Partial Class Workstations_Pilots
    ...
    Protected Sub gvRails_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvRails.RowCommand
        ...
        If e.CommandName = "gotoredtag" Then 'handles when the notes button is clicked
            Dim ID As String = e.CommandArgument 'get the row that is clicked
            Dim Selected As Integer = gvRails.Rows(ID).Cells(0).Text 'find the prod # in that row to use in the update command
            Dim workstation As String = "Pilots " 'to pass in the workstation that makes the note.
            'Response.Redirect("../BoatNotes.aspx?ws=" & workstation & "&ProdNo=" & Selected) 'This is commented out because Workstations can't set notes.
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        cmd3.CommandText = "Select Count (*) FROM BML_POPREPORTING Left Join BML_POPREPORTING_GREENLIGHTS on BML_POPREPORTING.ProdNo = BML_POPREPORTING_GREENLIGHTS.ProdNo where BML_POPREPORTING_GREENLIGHTS.BuildLoc = 'S' and RailsInstalledStatus = 1 and BML_POPREPORTING.ProdNo > '1910708'"
        cmd4.CommandText = "Select Count (*) FROM BML_POPREPORTING_GREENLIGHTS where IsPilot = 1 and ProdNo > '2106000' and IsInvoiced = 0"
    ...
    Protected Sub gvRails_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles gvRails.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            Dim x As SqlDataSource = CType(e.Row.FindControl("sdsInternalBuildDates"), SqlDataSource)
            Dim y As SqlDataSource = CType(e.Row.FindControl("sdsMacola"), SqlDataSource)
            Dim z As SqlDataSource = CType(e.Row.FindControl("sdsBoatTags"), SqlDataSource)
            ...
            Try 'use the selected prod # to get the green light values for the subgridview.  Used so that every greenlight section has headers.
                ...
                x.SelectParameters(0).DefaultValue = e.Row.Cells(0).Text
                y.SelectParameters(0).DefaultValue = so
                z.SelectParameters(0).DefaultValue = e.Row.Cells(0).Text
            ...
            'e.Row.Cells(8).ToolTip = "Rails"
            'e.Row.Cells(9).ToolTip = "Boat/Customer"
            'e.Row.Cells(10).ToolTip = "Custom"
            'e.Row.Cells(11).ToolTip = "Pilot"
            'e.Row.Cells(11).ToolTip = "Options"
            'e.Row.Cells(12).ToolTip = "Notes"
