<%@ Page Language="VB" AutoEventWireup="false" CodeFile="ReceivingQFronts.aspx.vb" Inherits="Rails_ReceivingQFronts" MaintainScrollPositionOnPostback="true" Debug="true" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title>RAILS Receiving Q</title>
...
<body>
 ...
 <div>
            <asp:Table ID="tblHeader" runat="server" CssClass="auto-style1" Width="1050px" GridLines="None">
                <asp:TableRow>
                    <asp:TableCell Wrap="false">
                        <span class="newStyle7">RAILS RECEIVING Q</span><br />
                        <asp:Button ID="btnRefresh" runat="server" Text="Refresh" />
                    </asp:TableCell>
                    <asp:TableCell>
                        <!-- #include file ="../Rails/toolbar-railsrcvq.aspx" -->
                    </asp:TableCell>
                </asp:TableRow>
            </asp:Table>          
            </div>
                         ...
                         <ItemTemplate>
                             <asp:GridView ID="GridView3" runat="server" AutoGenerateColumns="False" DataSourceID="sdsPopReportingByProdNo">
                                 <Columns>
                                     ...
                                     <asp:ImageField DataImageUrlField="QFrontStatus" DataImageUrlFormatString="../Images/{0}.png" HeaderText="Q FG">
                                         <ItemStyle HorizontalAlign="Center" />
                                     </asp:ImageField>
                                 </Columns>
                             </asp:GridView>
                             <asp:SqlDataSource ID="sdsPopReportingByProdNo" runat="server" ConnectionString="<%$ ConnectionStrings:bml_dataConnectionString %>" SelectCommand="SELECT [ProdNo], [FurnitureStatus],[FiberglassStatus], [CoversStatus], [RailsStatus], [TubesStatus], [QFrontStatus] FROM [BML_POPREPORTING_GREENLIGHTS] WHERE ([ProdNo] = @ProdNo)">
                                 <SelectParameters>
                         ...
                         <ItemTemplate>
                             <div class="newstyle2">
                                 ...
                                 <div class='<%# "checked" & Convert.ToBoolean(Eval("SF_Received"))%>'>  <asp:CheckBox class="railbuttons" ID="cbSF" runat="server" Text="SF" Font-Size="30" Checked='<%# Convert.ToBoolean(Eval("SF_Received"))%>'/></div>
                                  <div class='<%# "checked" & Convert.ToBoolean(Eval("PR_Received"))%>'> <asp:CheckBox class="railbuttons" ID="cbPR" runat="server" Text="PR" Font-Size="30" Checked='<%# Convert.ToBoolean(Eval("PR_Received"))%>' Visible="False" /></div>
                                  <div class='<%# "checked" & Convert.ToBoolean(Eval("SR_Received"))%>'> <asp:CheckBox class="railbuttons" ID="cbSR" runat="server" Text="SR" Font-Size="30" Checked='<%# Convert.ToBoolean(Eval("SR_Received"))%>' Visible="False" /></div>
                                  <div class='<%# "checked" & Convert.ToBoolean(Eval("T_Received"))%>'> <asp:CheckBox class="railbuttons" ID="cbT" runat="server" Text="T" Font-Size="30" Visible="False" Checked='<%# Convert.ToBoolean(Eval("T_Received"))%>'/></div>
                                  <div class='<%# "checked" & Convert.ToBoolean(Eval("PM_Received"))%>'> <asp:CheckBox class="railbuttons" ID="cbPM" runat="server" Text="PM" Font-Size="30" Visible="False" Checked ='<%# Convert.ToBoolean(Eval("PM_Received"))%>'/></div>
                                  <div class='<%# "checked" & Convert.ToBoolean(Eval("SM_Received"))%>'> <asp:CheckBox class="railbuttons" ID="cbSM" runat="server" Text="SM" Font-Size="30" Visible="False" Checked='<%# Convert.ToBoolean(Eval("SM_Received"))%>' /></div>
                                 &nbsp;</div>                             
                         </ItemTemplate>
        ...
        <asp:SqlDataSource ID="sdsPopReporting" runat="server" ConnectionString="<%$ ConnectionStrings:bml_dataConnectionString %>" SelectCommand="SELECT BML_POPREPORTING.ProdNo, BML_POPREPORTING.ItemNo, BML_POPREPORTING.IsCustom, BML_POPREPORTING.IsPilot, BML_POPREPORTING.CusName, BML_POPREPORTING.SONo, BML_POPREPORTING.BaseVinylColor, BML_POPREPORTING_GREENLIGHTS.RailsStatus, BML_POPREPORTING_GREENLIGHTS.BoatHasNotes, BML_POPREPORTING_GREENLIGHTS.NotesRailsPrep, BML_POPREPORTING.PanelColor, Rails_Tracking.WeldDate, Rails_Tracking.HasAllRails, Rails_Tracking.CustDrawNo, Rails_Tracking.PF_Received, Rails_Tracking.SF_Received, Rails_Tracking.PR_Received, Rails_Tracking.SR_Received, Rails_Tracking.T_Received, Rails_Tracking.PM_Received, Rails_Tracking.SM_Received, Rails_Tracking.NeedsTransom, Rails_Tracking.NeedsPortMid, Rails_Tracking.NeedsStrbMid, Rails_Tracking.RailsNeeded, Rails_Tracking.RepairFlag, Rails_Tracking.ReorderFlag,cast(Rails_Tracking.PF_Received as int) + cast(Rails_Tracking.SF_Received as int) as QFrontRailsTotal, BML_POPREPORTING_GREENLIGHTS.QFrontStatus FROM BML_POPREPORTING INNER JOIN BML_POPREPORTING_GREENLIGHTS ON BML_POPREPORTING.ProdNo = BML_POPREPORTING_GREENLIGHTS.ProdNo INNER JOIN Rails_Tracking ON BML_POPREPORTING.ProdNo = Rails_Tracking.ProdNo WHERE (BML_POPREPORTING_GREENLIGHTS.ProdNo LIKE '%' + @Search + '%') AND (BML_POPREPORTING.ItemNo LIKE '%' + @SearchBoat + '%' AND BML_POPREPORTING.ItemNo LIKE '%Q%' AND BML_POPREPORTING.ItemNo NOT LIKE '%QX%' ) AND (cast(Rails_Tracking.PF_Received as int) + cast(Rails_Tracking.SF_Received as int) <= 2) AND BML_POPREPORTING_GREENLIGHTS.FurnitureInstalledStatus < 2 ORDER BY Rails_Tracking.WeldDate desc, BML_POPREPORTING.ProdNo asc">
                <SelectParameters>                    
