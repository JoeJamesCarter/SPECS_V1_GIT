<%@ Page Language="VB" AutoEventWireup="false" CodeFile="Production.aspx.vb" Inherits="Rails_Production" MaintainScrollPositionOnPostback="true" Debug="true"  %>
...
<body>
 ...
 <div>
            <asp:Table ID="tblHeader" runat="server" CssClass="auto-style1" Width="1050px" GridLines="None">
                <asp:TableRow>
                    <asp:TableCell Wrap="false">
                        <span class="newStyle7">RAILS PRODUCING</span><br />
                ...
                <Columns>
                    ...
                    <asp:TemplateField HeaderText="Custom">
                     ...
                     <asp:TemplateField HeaderText="Rails">
                         <ItemTemplate>
                             <div class="newstyle2">
                                 <div class='<%# "checked" & Convert.ToBoolean(Eval("PF_Produced"))%>'> <asp:CheckBox class="railbuttons" ID="cbPF" runat="server" Text="PF" Font-Size="30pt" Checked='<%# Convert.ToBoolean(Eval("PF_Produced"))%>' AutoPostBack="True" OnCheckedChanged="cbPF_OnCheckedChanged" /></div>
                                 <div class='<%# "checked" & Convert.ToBoolean(Eval("SF_Produced"))%>'> <asp:CheckBox class="railbuttons" ID="cbSF" runat="server" Text="SF" Font-Size="30" Checked='<%# Convert.ToBoolean(Eval("SF_Produced"))%>' AutoPostBack="True" OnCheckedChanged="cbSF_OnCheckedChanged"/></div>
                                 <div class='<%# "checked" & Convert.ToBoolean(Eval("PR_Produced"))%>'> <asp:CheckBox class="railbuttons" ID="cbPR" runat="server" Text="PR" Font-Size="30" Checked='<%# Convert.ToBoolean(Eval("PR_Produced"))%>' AutoPostBack="True" OnCheckedChanged="cbPR_OnCheckedChanged"/></div>
                                 <div class='<%# "checked" & Convert.ToBoolean(Eval("SR_Produced"))%>'> <asp:CheckBox class="railbuttons" ID="cbSR" runat="server" Text="SR" Font-Size="30" Checked='<%# Convert.ToBoolean(Eval("SR_Produced"))%>' AutoPostBack="True" OnCheckedChanged="cbSR_OnCheckedChanged"/></div>
                                 <div class='<%# "checked" & Convert.ToBoolean(Eval("T_Produced"))%>'> <asp:CheckBox class="railbuttons" ID="cbT" runat="server" Text="T" Font-Size="30" Visible='<%# IIf(Eval("NeedsTransom").ToString() = "", 0, Eval("NeedsTransom")) = 1%>' Checked='<%# Convert.ToBoolean(Eval("T_Produced"))%>' AutoPostBack="True" OnCheckedChanged="cbT_OnCheckedChanged"/></div>
                                 <div class='<%# "checked" & Convert.ToBoolean(Eval("PM_Produced"))%>'> <asp:CheckBox class="railbuttons" ID="cbPM" runat="server" Text="PM" Font-Size="30" Visible='<%# IIf(Eval("NeedsPortMid").ToString() = "", 0, Eval("NeedsPortMid")) = 1%>' Checked ='<%# Convert.ToBoolean(Eval("PM_Produced"))%>' AutoPostBack="True" OnCheckedChanged="cbPM_OnCheckedChanged"/></div>
                                 <div class='<%# "checked" & Convert.ToBoolean(Eval("SM_Produced"))%>'> <asp:CheckBox class="railbuttons" ID="cbSM" runat="server" Text="SM" Font-Size="30" Visible='<%# IIf(Eval("NeedsStrbMid").ToString() = "", 0, Eval("NeedsStrbMid")) = 1%>' Checked='<%# Convert.ToBoolean(Eval("SM_Produced"))%>' AutoPostBack="True" OnCheckedChanged="cbSM_OnCheckedChanged"/></div>
                                 &nbsp;</div>                             
                         </ItemTemplate>
        ...
        <asp:SqlDataSource ID="sdsPopReporting" runat="server" ConnectionString="<%$ ConnectionStrings:bml_dataConnectionString %>" SelectCommand="SELECT BML_POPREPORTING.ProdNo, BML_POPREPORTING.ItemNo, BML_POPREPORTING.IsCustom, BML_POPREPORTING.IsPilot, BML_POPREPORTING.CusName, BML_POPREPORTING.SONo, BML_POPREPORTING.BaseVinylColor, BML_POPREPORTING_GREENLIGHTS.RailsStatus, BML_POPREPORTING_GREENLIGHTS.BoatHasNotes, BML_POPREPORTING_GREENLIGHTS.NotesRailsPrep, BML_POPREPORTING.PanelColor, Rails_Tracking.WeldDate, Rails_Tracking.HasAllRails, Rails_Tracking.CustDrawNo, Rails_Tracking.PF_Produced, Rails_Tracking.SF_Produced, Rails_Tracking.PR_Produced, Rails_Tracking.SR_Produced, Rails_Tracking.T_Produced, Rails_Tracking.PM_Produced, Rails_Tracking.SM_Produced, Rails_Tracking.NeedsTransom, Rails_Tracking.NeedsPortMid, Rails_Tracking.NeedsStrbMid, Rails_Tracking.RailsNeeded, Rails_Tracking.RepairFlag, Rails_Tracking.ReorderFlag, (CASE WHEN BML_POPREPORTING.ItemNo Like '%CL%' Then 1 ELSE 0 END) as ClubNeeded, (CASE WHEN BML_POPREPORTING.ItemNo Like '%SB%' AND BML_POPREPORTING.ItemNo Not Like '%SBRX%' Then 1 ELSE 0 END) as AftNeeded FROM BML_POPREPORTING INNER JOIN BML_POPREPORTING_GREENLIGHTS ON BML_POPREPORTING.ProdNo = BML_POPREPORTING_GREENLIGHTS.ProdNo INNER JOIN Rails_Tracking ON BML_POPREPORTING.ProdNo = Rails_Tracking.ProdNo WHERE (Rails_Tracking.WeldDate &lt;= @WeldDate) AND (Rails_Tracking.AllRailsSent = @AllRailsSent) AND (BML_POPREPORTING_GREENLIGHTS.RailsStatus < 2) And (BML_POPREPORTING_GREENLIGHTS.ProdNo LIKE '%' + @Search + '%') AND (BML_POPREPORTING.ItemNo LIKE '%' + @SearchBoat + '%') AND (BML_POPREPORTING.ItemNo NOT LIKE '%QX%') AND (BML_POPREPORTING.ItemNo NOT LIKE '25RX%') AND (BML_POPREPORTING.ItemNo NOT LIKE '23RX%') ORDER BY Rails_Tracking.WeldDate desc, BML_POPREPORTING.ProdNo asc">
                <SelectParameters>                    
                     <asp:ControlParameter ControlID="ddlDateSelected" Name="WeldDate" PropertyName="SelectedValue" DbType="Date" />                  
                    <asp:ControlParameter ControlID="ddlShowSelect" Name="AllRailsSent" PropertyName="SelectedValue" Type="Int32" />                     
