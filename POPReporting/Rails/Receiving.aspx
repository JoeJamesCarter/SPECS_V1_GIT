<%@ Page Language="VB" AutoEventWireup="false" CodeFile="Receiving.aspx.vb" Inherits="Rails_Receiving" MaintainScrollPositionOnPostback="true" Debug="true" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title>RAILS Receiving</title>
...
<body>
 ...
 <div>
            <asp:Table ID="tblHeader" runat="server" CssClass="auto-style1" Width="1050px" GridLines="None">
                <asp:TableRow>
                    <asp:TableCell Wrap="false">
                        <span class="newStyle7">RAILS RECEIVING</span><br />
                        <asp:Button ID="btnRefresh" runat="server" Text="Refresh" />
                    </asp:TableCell>
                    <asp:TableCell>
                        <!-- #include file ="../Rails/toolbar-railsrcv.aspx" -->
                    </asp:TableCell>
                   <%-- <asp:TableCell>
                       <asp:Label runat="server" Text="# Rails Rcvd Today:" CssClass="newStyle3"  Width="150px" ></asp:Label>
                    </asp:TableCell>
                    <asp:TableCell>
                        <asp:Label runat="server" ID="lblTotalRcvdToday" CssClass="newStyle3"></asp:Label>
                    </asp:TableCell>--%>
                </asp:TableRow>
            </asp:Table>          
            </div>
                ...
                <Columns>
                     ...
                     <asp:TemplateField HeaderText="Boat/Weld Date">
                         <ItemTemplate>
                             ...
                             <br />
                             <asp:Label ID="Label1" runat="server" CssClass="locs" Text='<%# "Loc:  " & Eval("RailsSlot")%>' Visible='<%# IIf(Eval("InUse").ToString() = "", 0, Eval("InUse")) = 1%>'></asp:Label>
                             <br>                             
                             <asp:Button ID="btnEditRailReqs" runat="server" CommandArgument='<%#Eval("ProdNo") %>' CommandName="gotoEditRailReqs" Text="Edit Requirements" />
                             </br>                             
                         </ItemTemplate>
                    ...
                    <asp:TemplateField HeaderText="Custom">
                     ...
                     <asp:TemplateField HeaderText="Options">
                         <ItemTemplate>
                             ...
                             <asp:SqlDataSource ID="sdsPopReportingFabOptions" runat="server" ConnectionString="<%$ ConnectionStrings:bml_dataConnectionString %>" SelectCommand="SELECT [ProdNo], [PopNo], [Option_Item], [Option_Desc] FROM [BML_POPREPORTINGFABOPTIONS] WHERE ([ProdNo] = @ProdNo) AND ([Workcenter] = @Workcenter) And OPTION_DESC not like 'PROTECH' ORDER BY [Option_Desc]">
                                 <SelectParameters>
                                     <asp:Parameter Name="ProdNo" Type="String" />
                                     <asp:Parameter DefaultValue="RailsPrep" Name="Workcenter" Type="String" />
                                 </SelectParameters>
                             </asp:SqlDataSource>                              
                             <asp:GridView ID="gvProtech" runat="server" GridLines="None" ShowHeader="False" AutoGenerateColumns="False" DataSourceID="sdsProtech" CssClass="newStyle3" BackColor="Orange"  >
                                 <Columns>
                                     <asp:BoundField DataField="Option_Desc" HeaderText="Fab Options" SortExpression="Option_Desc" ShowHeader="False" >
                                     <ControlStyle BorderStyle="None" />
                                     <FooterStyle BorderStyle="None" />
                             ...
                             <asp:SqlDataSource ID="sdsProtech" runat="server" ConnectionString="<%$ ConnectionStrings:bml_dataConnectionString %>" SelectCommand="SELECT t1.[ProdNo], [PopNo], [Option_Item], [Option_Desc] FROM [BML_POPREPORTINGFABOPTIONS] t1 Left join Rails_Tracking t2 on t1.ProdNo = t2.ProdNo WHERE (t1.ProdNo = @ProdNo) AND ([Workcenter] = @Workcenter) and t2.Protech = '1' And OPTION_DESC like 'PROTECH' ORDER BY [Option_Desc]">
                                 <SelectParameters>
                                     <asp:Parameter Name="ProdNo" Type="String" />
                                     <asp:Parameter DefaultValue="RailsPrep" Name="Workcenter" Type="String" />
                                 </SelectParameters>
                     ...
                     <asp:TemplateField HeaderText="Rails">
                         <ItemTemplate>
                             <div class="newstyle2">
                                 <div class='<%# "checked" & Convert.ToBoolean(Eval("PF_Received"))%>'> <asp:CheckBox class="railbuttons" ID="cbPF" runat="server" Text="PF" Font-Size="30pt" Checked='<%# Convert.ToBoolean(Eval("PF_Received"))%>'/></div>
                                 <div class='<%# "checked" & Convert.ToBoolean(Eval("SF_Received"))%>'>  <asp:CheckBox class="railbuttons" ID="cbSF" runat="server" Text="SF" Font-Size="30" Checked='<%# Convert.ToBoolean(Eval("SF_Received"))%>'/></div>
                                  <div class='<%# "checked" & Convert.ToBoolean(Eval("PR_Received"))%>'> <asp:CheckBox class="railbuttons" ID="cbPR" runat="server" Text="PR" Font-Size="30" Checked='<%# Convert.ToBoolean(Eval("PR_Received"))%>'/></div>
                                  <div class='<%# "checked" & Convert.ToBoolean(Eval("SR_Received"))%>'> <asp:CheckBox class="railbuttons" ID="cbSR" runat="server" Text="SR" Font-Size="30" Checked='<%# Convert.ToBoolean(Eval("SR_Received"))%>'/></div>
                                  <div class='<%# "checked" & Convert.ToBoolean(Eval("T_Received"))%>'> <asp:CheckBox class="railbuttons" ID="cbT" runat="server" Text="T" Font-Size="30" Visible='<%# IIf(Eval("NeedsTransom").ToString() = "", 0, Eval("NeedsTransom")) = 1%>' Checked='<%# Convert.ToBoolean(Eval("T_Received"))%>'/></div>
                                  <div class='<%# "checked" & Convert.ToBoolean(Eval("PM_Received"))%>'> <asp:CheckBox class="railbuttons" ID="cbPM" runat="server" Text="PM" Font-Size="30" Visible='<%# IIf(Eval("NeedsPortMid").ToString() = "", 0, Eval("NeedsPortMid")) = 1%>' Checked ='<%# Convert.ToBoolean(Eval("PM_Received"))%>'/></div>
                                  <div class='<%# "checked" & Convert.ToBoolean(Eval("SM_Received"))%>'> <asp:CheckBox class="railbuttons" ID="cbSM" runat="server" Text="SM" Font-Size="30" Visible='<%# IIf(Eval("NeedsStrbMid").ToString() = "", 0, Eval("NeedsStrbMid")) = 1%>' Checked='<%# Convert.ToBoolean(Eval("SM_Received"))%>' /></div>
                                 &nbsp;</div>                             
                         </ItemTemplate>
                         <ItemStyle VerticalAlign="Middle" Width="500px" />
                     </asp:TemplateField>                     
        ...
        <asp:SqlDataSource ID="sdsPopReporting" runat="server" ConnectionString="<%$ ConnectionStrings:bml_dataConnectionString %>" SelectCommand="SELECT BML_POPREPORTING.ProdNo, BML_POPREPORTING.ItemNo, BML_POPREPORTING.IsCustom, BML_POPREPORTING.IsPilot, BML_POPREPORTING.CusName, BML_POPREPORTING.SONo, BML_POPREPORTING.BaseVinylColor, BML_POPREPORTING_GREENLIGHTS.RailsStatus, BML_POPREPORTING_GREENLIGHTS.BoatHasNotes, BML_POPREPORTING_GREENLIGHTS.NotesRailsPrep, BML_POPREPORTING.PanelColor, Rails_Tracking.WeldDate, Rails_Tracking.HasAllRails, Rails_Tracking.CustDrawNo, Rails_Tracking.PF_Received, Rails_Tracking.SF_Received, Rails_Tracking.PR_Received, Rails_Tracking.SR_Received, Rails_Tracking.T_Received, Rails_Tracking.PM_Received, Rails_Tracking.SM_Received, Rails_Tracking.NeedsTransom, Rails_Tracking.NeedsPortMid, Rails_Tracking.NeedsStrbMid, Rails_Tracking.RailsNeeded, Rails_Tracking.RepairFlag, Rails_Tracking.ReorderFlag, Rails_Tracking.Requested, Rails_KittingLocations.RailsSlot, Rails_KittingLocations.InUse FROM BML_POPREPORTING INNER JOIN BML_POPREPORTING_GREENLIGHTS ON BML_POPREPORTING.ProdNo = BML_POPREPORTING_GREENLIGHTS.ProdNo INNER JOIN Rails_Tracking ON BML_POPREPORTING.ProdNo = Rails_Tracking.ProdNo LEFT JOIN Rails_KittingLocations on BML_POPREPORTING.ProdNo = Rails_KittingLocations.ProdNo WHERE (Rails_Tracking.WeldDate &lt;= @WeldDate) AND (Rails_Tracking.HasAllRails = @HasAllRails) AND (BML_POPREPORTING_GREENLIGHTS.RailsStatus = @RailsStatus) And (BML_POPREPORTING_GREENLIGHTS.ProdNo LIKE '%' + @Search + '%') AND ((BML_POPREPORTING.ItemNo LIKE '%' + @SearchBoat + '%') AND (BML_POPREPORTING.ItemNo NOT LIKE '%QX%')) AND Convert(date,WeldDate) &gt;= '06/12/2017' ORDER BY Rails_Tracking.WeldDate desc, BML_POPREPORTING.ProdNo asc">
                <SelectParameters>                    
                    ...
                    <asp:ControlParameter ControlID="ddlRailsStatus" Name="RailsStatus" PropertyName="SelectedValue" Type="Int32" />                     
