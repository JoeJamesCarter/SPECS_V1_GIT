<%@ Page Language="VB" AutoEventWireup="false" CodeFile="TubesLocation.aspx.vb" Inherits="TubesLocation" MaintainScrollPositionOnPostback="true" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title>TUBES BY LOCATION</title>
...
<body>
 ...
 <div>
            <asp:Table ID="tblHeader" runat="server" CssClass="auto-style1" Width="1050px" GridLines="None">
                <asp:TableRow>
                    <asp:TableCell Wrap="false">
                        <span class="newStyle7">TUBES BY LOCATION</span><br />
                        <asp:Button ID="btnRefresh" runat="server" Text="Refresh" />
                    </asp:TableCell>
                    <asp:TableCell>
                       <!-- #include file ="toolbar-tubeslocation.aspx" -->
                    </asp:TableCell>
                </asp:TableRow>
            </asp:Table>          
            <asp:HiddenField ID="tstatus" runat="server" />
            <asp:HiddenField ID="fstatus" runat="server" />
            </div>
        <asp:GridView ID="gvAllFieldsT" runat="server" AutoGenerateColumns="False" DataSourceID="sdsPopReporting" Height="177px" CellPadding="4" BorderColor="#999999" BorderStyle="Solid" BorderWidth="3px" CellSpacing="2" CssClass="newStyle1" ForeColor="Black" DataKeyNames="ProdNo">
                <Columns>
                     ...
                     <asp:TemplateField HeaderText="Green Lights">
                      ...
                      <asp:TemplateField HeaderText="Floors Status">
                    <ItemTemplate>
                        <asp:ImageButton ID="btnUpdateFloors" runat="server"  Enabled ="false"  ImageUrl='<%# Eval("FloorsInstalledStatus", "./Images/framescomplete_status{0}.png") %>' CommandArgument="<%# Container.DataItemIndex %>" CommandName="gotoupdate"/>                                           
                    </ItemTemplate>
                          <ItemStyle HorizontalAlign="Center" VerticalAlign="Middle" />
                </asp:TemplateField>
                     <asp:TemplateField HeaderText="Update">
                         <ItemTemplate>
                             <div class="newStyle2">
                             <asp:RadioButtonList ID="rblTubesLocationStatus" runat="server" SelectedValue='<%# Bind("TubesOutside")%>' RepeatDirection="Horizontal">
                                 <asp:ListItem Value="0">Inside</asp:ListItem>                                 
                                 <asp:ListItem Value="1">Outside</asp:ListItem>
                             </asp:RadioButtonList>
                                 </div>
                             <div class="newStyle2">
                                 <asp:Button ID="btnUpdateTubesStatus" runat="server" Text="Update"  CommandArgument="<%# Container.DataItemIndex %>" CommandName="UpdateTLocStatus"/>
                             </div>
                         </ItemTemplate>
                         <ItemStyle VerticalAlign="Middle" />
                     </asp:TemplateField>
                     <asp:ImageField DataImageUrlField="TubesStatus" DataImageUrlFormatString="./Images/{0}checkmark.png" HeaderText="Complete" Visible="False">
                         <ItemStyle HorizontalAlign="Center" VerticalAlign="Middle" />
                     </asp:ImageField>
        ...
        <asp:SqlDataSource ID="sdsPopReporting" runat="server" ConnectionString="<%$ ConnectionStrings:bml_dataConnectionString %>" SelectCommand="SELECT BML_POPREPORTING.ProdNo, BML_POPREPORTING.ItemNo,  BML_POPREPORTING.IsCustom, BML_POPREPORTING.CusName, BML_POPREPORTING.IsCustom, BML_POPREPORTING.SONo, BML_POPREPORTING.BaseVinylColor, BML_POPREPORTING_GREENLIGHTS.TubesStatus, BML_POPREPORTING_GREENLIGHTS.TubesOutside, BML_POPREPORTING_GREENLIGHTS.CentertubeStatus, BML_POPREPORTING_GREENLIGHTS.FloorsInstalledStatus, BML_POPREPORTING_GREENLIGHTS.CentertubeNeeded, BML_POPREPORTING_GREENLIGHTS.BoatHasNotes, BML_POPREPORTING_GREENLIGHTS.NotesTubes FROM BML_POPREPORTING CROSS JOIN BML_POPREPORTING_GREENLIGHTS WHERE(BML_POPREPORTING.ProdNo = BML_POPREPORTING_GREENLIGHTS.ProdNo) AND (BML_POPREPORTING_GREENLIGHTS.CompleteStatus = @CompleteStatus) AND (BML_POPREPORTING.DueDate &lt;= @DueDate) AND (BML_POPREPORTING_GREENLIGHTS.TubesOutside &lt; @TubesOutside) AND (BML_POPREPORTING_GREENLIGHTS.TubesStatus = @TStatus) AND (BML_POPREPORTING_GREENLIGHTS.FramesInstalledStatus &lt; @FStatus) AND (BML_POPREPORTING_GREENLIGHTS.ProdNo LIKE '%' + @Search + '%') AND (BML_POPREPORTING.ItemNo LIKE '%' + @SearchBoat + '%') ORDER BY BML_POPREPORTING.ProdNo">
                <SelectParameters>
                    ...
                    <asp:ControlParameter ControlID="ddlShowSelect" Name="TubesOutside" PropertyName="SelectedValue" Type="Int32" />
                    <asp:ControlParameter ControlID="tstatus" Name="TStatus" DefaultValue = '2' Type="Int32" />
                    <asp:ControlParameter ControlID="fstatus" Name="FStatus" DefaultValue = '2' Type="Int32" />
