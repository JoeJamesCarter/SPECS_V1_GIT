<%@ Page Language="VB" AutoEventWireup="false" CodeFile="ProductDesignIdeas.aspx.vb" MaintainScrollPositionOnPostback="true" Inherits="PDI_ProductDesignIdeas" Debug="true" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title>Product Design Ideas</title>
...
<body>
    <form id="form1" runat="server">
        <div>
            <asp:HyperLink runat="server" NavigateUrl="http://pdi/">PDI Moved To A New Link - Click Here.</asp:HyperLink>
            <%--<asp:Label ID="Label2" runat="server" CssClass="newStyle7" Text="PRODUCT DEVELOPMENT IDEAS"></asp:Label>
            &nbsp;&nbsp;&nbsp;&nbsp;<asp:Button ID="btnRefresh" runat="server" Text="Refresh" />&nbsp;&nbsp;&nbsp;&nbsp;<asp:Button ID="btnAdd" runat="server" Text="Add An Idea" />
            &nbsp;&nbsp;
            <asp:Button ID="Button1" runat="server" Text="View History" />
            &nbsp;&nbsp;
            <asp:Button ID="btnRepIdeas" runat="server" Text="View Rep Ideas" />
&nbsp;&nbsp;
            <asp:Label ID="Label11" runat="server" CssClass="newStyle6" Text="# of Ideas:"></asp:Label>
&nbsp;
            <asp:Label ID="lblIdeaCount" runat="server" CssClass="newStyle6"></asp:Label>
            <br />
            <br />
        ...
        <asp:DropDownList ID="ddlMY" runat="server" CssClass="newStyle6" AutoPostBack="True">
            ...
            <asp:Label ID="Label5" runat="server" CssClass="filterlabels" Text="Approval Status"></asp:Label>
            <asp:DropDownList ID="ddlApprovalStatuses" runat="server" AppendDataBoundItems="True" AutoPostBack="True" DataSourceID="sdsDistinctApprovalStatuses" DataTextField="ApprovalStatusDesc" DataValueField="AutoID" CssClass="newStyle6">
                <asp:ListItem Text="Show All" Value="%" />
            </asp:DropDownList>
            ...
            <asp:SqlDataSource ID="sdsDistinctApprovalStatuses" runat="server" ConnectionString="<%$ ConnectionStrings:bml_dataConnectionString %>" SelectCommand="SELECT [ApprovalStatusDesc], AutoID FROM [ProductDesignIdeas_ApprovalStatuses] ORDER BY [ApprovalStatusDesc]"></asp:SqlDataSource>
            <br />
            <br />
            <asp:GridView ID="gvProjectIdeas" runat="server" DataSourceID="sdsProductDevIdeas" AutoGenerateColumns="False" CssClass="newStyle1" DataKeyNames="ModelYear,AutoID" BackColor="White" BorderColor="#999999" BorderStyle="Solid" BorderWidth="1px" CellPadding="3" ForeColor="Black" GridLines="Vertical" AllowSorting="True">
                ...
                <Columns>
                    ...
                    <asp:TemplateField HeaderText ="Rep Votes">
                        <ItemTemplate>
                            <asp:Table ID="Table1" runat="server" BorderWidth="2" BorderStyle="Solid" GridLines="Both">
                                <asp:TableHeaderRow>
                                    <asp:TableHeaderCell>Agree</asp:TableHeaderCell>
                                    <asp:TableHeaderCell>Neutral</asp:TableHeaderCell>
                                    <asp:TableHeaderCell>Disagree</asp:TableHeaderCell>
                                </asp:TableHeaderRow>
                                <asp:TableRow>
                                    <asp:TableCell HorizontalAlign="Center"><asp:Label ID="lblAgree" runat="server" Text=" "></asp:Label></asp:TableCell>
                                    <asp:TableCell HorizontalAlign="Center"><asp:Label ID="lblNeutral" runat="server" Text=" "></asp:Label></asp:TableCell>
                                    <asp:TableCell HorizontalAlign="Center"><asp:Label ID="lblDisagree" runat="server" Text=" "></asp:Label></asp:TableCell>
                                </asp:TableRow>
                            </asp:Table>
                        </ItemTemplate>
                    </asp:TemplateField>                                    
            ...
            <asp:SqlDataSource ID="sdsProductDevIdeas" runat="server" ConnectionString="<%$ ConnectionStrings:bml_dataConnectionString %>" DeleteCommand="DELETE FROM [ProductDesignIdeas] WHERE [ModelYear] = @ModelYear AND [AutoID] = @AutoID" InsertCommand="INSERT INTO [ProductDesignIdeas] ([ModelYear], [AutoID], [RequestedBy], [RequestedDate], [ApprovalStatus], [Category], [Series], [InitialDescription], [NarrowedDescription], [StrategicPriority], [SalesPriority], [EngActiveFlag], [ProjectLead], [TargetDate], [ProjectStatus], [Notes]) VALUES (@ModelYear, @AutoID, @RequestedBy, @RequestedDate, @ApprovalStatus, @Category, @Series, @InitialDescription, @NarrowedDescription, @StrategicPriority, @SalesPriority, @EngActiveFlag, @ProjectLead, @TargetDate, @ProjectStatus, @Notes)" SelectCommand="SELECT * FROM [ProductDesignIdeas] WHERE ([ModelYear] Like @ModelYear) and ([RequestedBy] like @RequestedBy OR [RequestedBy] is NULL) and ([Category] like @Category or [Category] is Null) and ([Series] like @Series or [Series] is Null) and [EngActiveFlag] Like @EngActiveFlag and ([ProjectStatus] like @ProjectStatus or [ProjectStatus] is Null) and ([ApprovalStatus] like @ApprovalStatus or [ApprovalStatus] is Null) and [AutoID] like @ProjectID and ([ProjectStatus] <> 2 and [ApprovalStatus] <> 1 and [ApprovalStatus] <> 4) ORDER BY [ModelYear], [AutoID]" UpdateCommand="UPDATE [ProductDesignIdeas] SET [RequestedBy] = @RequestedBy, [RequestedDate] = @RequestedDate, [ApprovalStatus] = @ApprovalStatus, [Category] = @Category, [Series] = @Series, [InitialDescription] = @InitialDescription, [NarrowedDescription] = @NarrowedDescription, [StrategicPriority] = @StrategicPriority, [SalesPriority] = @SalesPriority, [EngActiveFlag] = @EngActiveFlag, [ProjectLead] = @ProjectLead, [TargetDate] = @TargetDate, [ProjectStatus] = @ProjectStatus, [Notes] = @Notes WHERE [ModelYear] = @ModelYear AND [AutoID] = @AutoID">
                ...
                <SelectParameters>
                    ...
                    <asp:ControlParameter ControlID="ddlApprovalStatuses" Name="ApprovalStatus" PropertyName="SelectedValue" DefaultValue='%' Type="String" />
