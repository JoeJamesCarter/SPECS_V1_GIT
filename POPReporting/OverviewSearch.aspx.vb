...
Partial Class OverviewSearch
    ...
    Private Shared Sub AddSuperHeader(gridView As GridView)
        ...
        cmd6.CommandText = "Select Count (*) FROM BML_POPREPORTING_GREENLIGHTS where FramesInstalledStatus = 2 and ProdLineID = 3 and Convert(date,FramesInstalled) ='" & ThisDay & "'"
        cmd7.CommandText = "Select Count (*) FROM BML_POPREPORTING_GREENLIGHTS where FloorsInstalledStatus = 2 and ProdLineID = 3 and Convert(date,FloorsInstalled) ='" & ThisDay & "'"
        cmd8.CommandText = "Select Count (*) FROM BML_POPREPORTING_GREENLIGHTS where RailsInstalledStatus = 2 and ProdLineID = 3 and Convert(date,RailsInstalled) ='" & ThisDay & "'"
        cmd9.CommandText = "Select Count (*) FROM BML_POPREPORTING_GREENLIGHTS where FurnitureInstalledStatus = 2 and ProdLineID = 3 and Convert(date,FurnitureInstalled) ='" & ThisDay & "'"
        cmd10.CommandText = "Select Count (*) FROM BML_POPREPORTING_GREENLIGHTS where HelmInstalledStatus = 2 and ProdLineID = 3 and Convert(date,HelmInstalled) ='" & ThisDay & "'"
        cmd11.CommandText = "Select Count (*) FROM BML_POPREPORTING_GREENLIGHTS where CleaningStatus = 2 and ProdLineID = 3 and Convert(date,Cleaning) ='" & ThisDay & "'"
        cmd12.CommandText = "Select Count (*) FROM BML_POPREPORTING_GREENLIGHTS where ShrinkwrapStatus = 2 and ProdLineID =3 and Convert(date,Shrinkwrap) ='" & ThisDay & "'"
        cmd13.CommandText = "Select Count (*) FROM BML_POPREPORTING_GREENLIGHTS where WavetamerStatus = 2 and ProdLineID = 3 and Convert(date,WaveTamerInstalled) ='" & ThisDay & "'"
        cmd14.CommandText = "Select Count (*) FROM BML_POPREPORTING_GREENLIGHTS where FinalInspectionStatus = 2 and ProdLineID = 3 and Convert(date,FinalInspection) ='" & ThisDay & "'"
        cmd15.CommandText = "Select Count (*) FROM BML_POPREPORTING_GREENLIGHTS where SharkhideStatus = 2 and ProdLineID = 3 and Convert(date,SharkhideInstalled) ='" & ThisDay & "'"
        cmd16.CommandText = "Select Count (*) FROM BML_POPREPORTING_GREENLIGHTS where CompleteStatus = 2 and ProdLineID = 3 and Convert(date,Complete) ='" & ThisDay & "'"
        cmd17.CommandText = "Select Count (*) FROM BML_POPREPORTING_OFFLINE where StatusSetTo = 'Offline' and ProdLineID = 3 and Convert(date,Date) ='" & ThisDay & "'"
        cmd18.CommandText = "Select Count (*) FROM BML_POPREPORTING_GREENLIGHTS where CentertubeStatus = '2' and ProdLineID = 3 and Convert(date,Centertube) ='" & ThisDay & "'"
        cmd19.CommandText = "Select Count (*) FROM BML_POPREPORTING_GREENLIGHTS where RiggingStatus = '2' and ProdLineID = 3 and Convert(date,Rigging) ='" & ThisDay & "'"
    ...
    Protected Sub gvOverview_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles gvOverview.RowDataBound
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            e.Row.Cells(10).ToolTip = "Q Front FG"
            e.Row.Cells(11).ToolTip = "Q Front Rails"
            e.Row.Cells(12).ToolTip = "Arch/Tower"
            e.Row.Cells(13).ToolTip = "Frames"
            e.Row.Cells(14).ToolTip = "Boat/Customer"
            e.Row.Cells(15).ToolTip = "Frames"
            e.Row.Cells(16).ToolTip = "Floors"
            e.Row.Cells(17).ToolTip = "Trim"
            e.Row.Cells(18).ToolTip = "Rails"
            e.Row.Cells(19).ToolTip = "Furniture"
            e.Row.Cells(20).ToolTip = "Helm"
            e.Row.Cells(21).ToolTip = "Inspection"
            e.Row.Cells(22).ToolTip = "Final Inspection"
            e.Row.Cells(23).ToolTip = "Shrinkwrap"
            e.Row.Cells(24).ToolTip = "Rigging"
            e.Row.Cells(25).ToolTip = "Wavetamer"
            e.Row.Cells(26).ToolTip = "Sharkhide"
            e.Row.Cells(27).ToolTip = "Rigging"
            e.Row.Cells(28).ToolTip = "Complete"
            e.Row.Cells(29).ToolTip = "Offline"
            e.Row.Cells(30).ToolTip = "Notes"
    ...
    Protected Sub gvOverview_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvOverview.RowCommand
        ...
        If e.CommandName = "gotoredtag" Then 'handles when the notes button is clicked
            ...
            Dim Selected As Integer = gvOverview.Rows(ID).Cells(0).Text 'find the prod # in that row to use in the update command
            Dim workstation As String = "OverviewSearch " 'to pass in the workstation that makes the note.
            ...
            Response.Write("<script language='javascript'> {popUpObj=window.open ('" & url & "' ,'mywindow','menubar=0,resizable=1,width=1024,height=800,toolbars=0');popUpObj.focus()}</script>")
