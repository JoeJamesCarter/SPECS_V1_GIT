...
Partial Class Tubes_Replacements
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If ddlShowSelect.SelectedValue = 1 Then
            gvTubeReplacements.Columns(6).Visible = False
        Else : gvTubeReplacements.Columns(6).Visible = True
        End If
        ...
        cmd.CommandText = "Select Count (*) FROM Tubes_DamageReported where ReplacedFlag = 1 and Convert(date,Replaced) ='" & ThisDay & "'"
        cmd.Connection = conn
    ...
    Protected Sub ddlShowSelect_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ddlShowSelect.SelectedIndexChanged
        gvTubeReplacements.DataBind() 'used to allow the user to hide/show jobs that they have completed.
        If ddlShowSelect.SelectedValue = 1 Then
            gvTubeReplacements.Columns(6).Visible = False
        Else : gvTubeReplacements.Columns(6).Visible = True
        End If
    ...
    Protected Sub gvTubeReplacements_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvTubeReplacements.RowCommand
        If e.CommandName = "gotoupdate" Then
            ...
            Dim img As Image = CType(gvTubeReplacements.Rows(Key).FindControl("btnUpdateReplaced"), Image)
            ...
            cmd1.CommandText = "UPDATE Tubes_DamageReported Set ReplacedFlag = 1, Replaced = '" & TodaysDate & "' Where AutoID = '" & TubeID & "'"

            'count completed today
            cmd2.CommandText = "Select Count (*) FROM Tubes_DamageReported where ReplacedFlag = 1 and Convert(date,Replaced) ='" & ThisDay & "'"

            'reverse done
            cmd4.CommandText = "UPDATE BML_POPREPORTING_GREENLIGHTS SET HasTubeDamage = 1 where ProdNo =  '" & SelectedProd & "'"
            cmd3.CommandText = "UPDATE Tubes_DamageReported Set ReplacedFlag = 0, Replaced = NULL Where AutoID = '" & TubeID & "'"
            ...
            Try
                ...
                mail.Subject = "Tube Replacement Completed - " & SelectedProd
                mail.IsBodyHtml = True
                Dim mymessage As String = "Tube replacement has been completed on " & TodaysDate & newline & "Prod #: " & SelectedProd & newline & "Model: " & SelectedModel & newline & "Tube: " & Tube & newline & "Completed By: " & stationID & newline & ""
    ...
    Protected Sub gvTubeReplacements_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles gvTubeReplacements.RowDataBound
