...
Partial Class Tubes_BlueRoller
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        cmd2.CommandText = "Select Count (*) FROM Tubes where RollerPStatus = 2 and OuterRoller = 'Blue' and Convert(date,RollerP) ='" & ThisDay & "'"
        cmd3.CommandText = "Select Count (*) FROM Tubes where RollerSStatus = 2 and OuterRoller = 'Blue' and Convert(date,RollerS) ='" & ThisDay & "'"
        cmd4.CommandText = "Select Count (*) FROM Tubes where RollerCStatus = 2 and CenterRoller = 'Blue' and Convert(date,RollerC) ='" & ThisDay & "'"
    ...
    Protected Sub gvRoller_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvRoller.RowCommand
        ...
        If e.CommandName = "gotoupdateP" Then
            ...
            cmd2.CommandText = "Select Count (*) FROM Tubes where RollerPStatus = 2 and RollerSStatus = 2 and OuterRoller = 'Blue' and Convert(date,RollerP) ='" & ThisDay & "'"
            ...
            cmd9.CommandText = "Select Count (*) FROM Tubes where RollerPStatus = 2 and OuterRoller = 'Blue' and Convert(date,RollerP) ='" & ThisDay & "'"
            cmd10.CommandText = "Select Count (*) FROM Tubes where RollerSStatus = 2 and OuterRoller = 'Blue' and Convert(date,RollerS) ='" & ThisDay & "'"
            cmd11.CommandText = "Select Count (*) FROM Tubes where RollerCStatus = 2 and CenterRoller = 'Blue' and Convert(date,RollerC) ='" & ThisDay & "'"
        ...
        If e.CommandName = "gotoupdateCtr" Then
            ...
            cmd2.CommandText = "Select Count (*) FROM Tubes where RollerStatusCtr = 2 and Convert(date,RollerC) ='" & ThisDay & "'"
            'Determine if the next workstation already has a done date set.
    ...
    Protected Function PrintLabelsByTubeType(prod As String, numsections As Integer, tube As String, tubetype As String) As String
        ...
        Dim ipAddress As String = "10.54.80.17" 'Blue
        'Dim ipAddress As String = "10.5.110.117" 'Blue on 5-21-20
            ...
            ElseIf tubetype = "rerun" Then
                ...
                ZPLString2x1 =
                   "^XA~TA000~JSN^LT0^MNW^MTT^PON^PMN^LH0,0^JMA^PR6,6~SD15^JUS^LRN^CI0^XZ" &
                    ...
                    "^LL0203" &
                    "^LS0" &
                    "^FO160,160^GFA,00768,00768,00012,:Z64:" &
                    "eJztjjEKwzAMRV00aAnSmsGNzhHopbp3SPHgsVdKphyj9BYZgtyv9A4dSmU+PMzn8VP6nbPdmu1C9TQnc3VzoXKfkjVt1oTXZU6dSoQqOHNGPtyTRKi8pviLcL3OST0eOmD4/fCP4A3+Df0R/ZtEeD3DeZEIVQUP8A96cC/wizIx+qotKzdm7OGHd1ycKfY8W+ZlZ+z/33fuDU7XOqw=:5EE3" &
                    "^BY2,3,121^FT13,164^BCN,,Y,Y" &
                    "^FD>;" &
                    prodfirst6 &
                    ">" &
                    "6" &
    ...
    Protected Function PrintBuildInstrLabelsCustService(prod As String) As String
        Dim ipAddress As String = "10.54.80.17" 'Blue
        'Dim ipAddress As String = "10.5.110.117" 'Blue
        Dim port As Integer = 9100
    ...
    Protected Sub gvCSTubes_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvCSTubes.RowCommand
        If e.CommandName = "gotoupdateCSPort" Then
            ...
            cmd7.CommandText = "UPDATE CS_Tubes SET RollerPStatus = '2', SeamerSectPFStatus = @PF,  SeamerSectPMStatus = @PM, SeamerSectPM2Status = @PM2, SeamerSectPRStatus = @PR, FinalPStatus = 1, RollerP = '" & TodaysDate & "', OuterRollerID = '" & stationID & "' where TubeID =  '" & SelectedTubeID & "'"
            'reversal
            cmd8.CommandText = "UPDATE CS_Tubes SET RollerPStatus = '1', SeamerSectPFStatus = @PF8,  SeamerSectPMStatus = @PM8, SeamerSectPM2Status = @PM28, SeamerSectPRStatus = @PR8, RollerP = '" & TodaysDate & "' where TubeID =  '" & SelectedTubeID & "'"
            ...
            If framevalue <= 1 Then
                cmd7.ExecuteNonQuery()
                PrintLabelsByTubeType(SelectedTubeID, NumOuterSections, "Port", "custserv")
                Try
        ...
        If e.CommandName = "gotoupdateCSStrb" Then
            ...
            cmd7.CommandText = "UPDATE CS_Tubes SET RollerSStatus = '2', SeamerSectSFStatus = @SF,  SeamerSectSMStatus = @SM, SeamerSectSM2Status = @SM2, SeamerSectSRStatus = @SR, FinalSStatus = 1, RollerS = '" & TodaysDate & "', OuterRollerID = '" & stationID & "' where TubeID =  '" & SelectedTubeID & "'"
            'reversal
            cmd8.CommandText = "UPDATE CS_Tubes SET RollerSStatus = '1', SeamerSectSFStatus = @SF8,  SeamerSectSMStatus = @SM8, SeamerSectSM2Status = @SM28, SeamerSectSRStatus = @SR8, RollerS = '" & TodaysDate & "' where TubeID =  '" & SelectedTubeID & "'"
            ...
            If framevalue <= 1 Then
                cmd7.ExecuteNonQuery()
                PrintLabelsByTubeType(SelectedTubeID, NumOuterSections, "Strb", "custserv")
                Try
                    PrintBuildInstrLabelsCustService(SelectedTubeID)
                Catch ex As Exception
