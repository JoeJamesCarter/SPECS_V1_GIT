<%@ Page Language="VB" AutoEventWireup="false" CodeFile="Replacements.aspx.vb" Inherits="Tubes_Replacements" %>
...
<body>
    ...
    <div>   
          <asp:Table ID="tblHeader" runat="server" CssClass="auto-style1" Width="1024px" GridLines="None">
                <asp:TableRow>
                    <asp:TableCell>
                        <span class="newStyle7">TUBE REPLACEMENTS</span><br />
        ...
        <br/>
        <div id="viewport">
            <asp:GridView ID="gvTubeReplacements" runat="server" AutoGenerateColumns="False" CssClass="newStyle1" DataSourceID="sdsTubeReplacements" DataKeyNames="AutoID" Width="1000px">
                <Columns>                    
                    ...
                    <asp:TemplateField HeaderText ="Damage Description">
                        <ItemTemplate>
                           ...
                           <br /></strong>
                            ...
                            Reported: <asp:Label ID="Label2" runat="server" Text='<%# Eval("DateReported", "{0:d hh:mm}")%>'></asp:Label><br />
                            ReRolled: <asp:Label ID="Label19" runat="server" Text='<%# Eval("ReRolled", "{0:M/d hh:mm}")%>'></asp:Label>
                        </ItemTemplate>
                        <ItemStyle HorizontalAlign="Center" />
                    </asp:TemplateField> 
                    ...
                    <asp:BoundField DataField="Replaced" HeaderText=" Date Replaced" SortExpression="Replaced" />
                    <asp:TemplateField>
                        <ItemTemplate>
                            <asp:ImageButton ID="btnUpdateReplaced" runat="server"  Enabled =' <%# Eval("ReplacedFlag") >= 0%>' ImageUrl='<%# Eval("ReplacedFlag", "../Images/replaced{0}.png")%>' CommandArgument='<%# Container.DataItemIndex & ";" & Eval("AutoID") & ";" & Eval("ProdNo") & ";" & Eval("ItemNo") & ";" & Eval("Tube")%>' CommandName="gotoupdate"/>                                           
                        </ItemTemplate>
                        <ItemStyle HorizontalAlign="Center" />
                    </asp:TemplateField>                    
                </Columns>
            </asp:GridView>
            <asp:SqlDataSource ID="sdsTubeReplacements" runat="server" ConnectionString="<%$ ConnectionStrings:bml_dataConnectionString %>" SelectCommand="SELECT Tubes_DamageReported.*, ItemNo, BuildLoc, SONo, BML_POPREPORTING_GREENLIGHTS.DueDate, Yard_Master_Locations.SlotKey, Filled, CusName, ActualLoadDate, ReRolled, Case When ActualLoadDate is NULL then '0' ELSE '1' End As ScheduledToLoad, CASE When BML_POPREPORTING_GREENLIGHTS.CompleteStatus = '2' Then 'Complete' WHEN BML_POPREPORTING_GREENLIGHTS.SharkhideStatus = '1' Then 'Sharkhide' When BML_POPREPORTING_GREENLIGHTS.WavetamerStatus ='1' Then 'Wavetamer' When BML_POPREPORTING_GREENLIGHTS.RiggingStatus ='1' Then 'Rigging' When BML_POPREPORTING_GREENLIGHTS.ShrinkwrapStatus ='1' Then 'Shrinkwrap' When BML_POPREPORTING_GREENLIGHTS.CleaningStatus ='1' Then 'Inspection' When BML_POPREPORTING_GREENLIGHTS.FinalInspectionStatus ='1' Then 'Final' When BML_POPREPORTING_GREENLIGHTS.HelmInstalledStatus ='1' Then 'Helm' When BML_POPREPORTING_GREENLIGHTS.FurnitureInstalledStatus ='1' Then 'Furniture' When BML_POPREPORTING_GREENLIGHTS.RailsInstalledStatus ='1' Then 'Rails' When BML_POPREPORTING_GREENLIGHTS.FloorsInstalledStatus ='1' Then 'Floors' When BML_POPREPORTING_GREENLIGHTS.FramesInstalledStatus ='1' Then 'Frames' End as CurrentStatus FROM [Tubes_DamageReported] Left Join BML_POPREPORTING_GREENLIGHTS on Tubes_DamageReported.ProdNo = BML_POPREPORTING_GREENLIGHTS.ProdNo left join Yard_Master_Locations On Tubes_DamageReported.ProdNo = Yard_Master_Locations.ProdNo Left Join Load_Scheduling on Tubes_DamageReported.ProdNo = Load_Scheduling.ProdNo WHERE ([ReplaceReqdFlag] = @ReplaceReqdFlag) And (ReplacedFlag &lt; @ReplacedFlag) AND (Tubes_DamageReported.ProdNo LIKE '%' + @Search + '%') AND (BML_POPREPORTING_GREENLIGHTS.ItemNo LIKE '%' + @SearchBoat + '%') And ReportedBy <> 'specstube1' ORDER BY ScheduledToLoad desc, ActualLoadDate, CusName, DueDate, [DateEvaluated]">
                <SelectParameters>
                    <asp:Parameter DefaultValue="1" Name="ReplaceReqdFlag" Type="Int32" />
                    <asp:ControlParameter ControlID="ddlShowSelect" Name="ReplacedFlag" PropertyName="SelectedValue" Type="Int32" DefaultValue="0" />
